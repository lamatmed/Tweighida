...
export default function NoteList({ initialNotes }: { initialNotes: Note[] }) {
  ...
  const generatePDF = () => {
    ...
    // ✅ Totaux améliorés
    doc.setFontSize(11);
    doc.setTextColor(0, 0, 128); // Bleu foncé

    doc.text("Resumo das Vendas", 14, finalY + 10);

    doc.setFontSize(10);
    doc.setTextColor(0, 0, 0); // Noir normal
    doc.text(`• Total de Vendas: ${totalVenda.toLocaleString('pt-PT', { minimumFractionDigits: 2 })} KZ`, 14, finalY + 20);
    doc.text(`• Total 7% de Vendas: ${total7Percent.toLocaleString('pt-PT', { minimumFractionDigits: 2 })} KZ`, 14, finalY + 28);
    doc.text(`• Total de Classificações: ${filteredNotes.length}`, 14, finalY + 36);
    ...
    doc.text(`Data de Geração: ${dateGeneration} ${timeGeneration} — NotesApp V1.0.0`, 14, finalY + 46);
